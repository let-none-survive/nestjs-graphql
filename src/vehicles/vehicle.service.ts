...
import { Vehicle as VehicleSchema, VehicleDocument } from './schemas/vehicle.schema';

@Injectable()
export class VehicleService {
  constructor(@InjectModel(VehicleSchema.name) private vehiclesModel: Model<VehicleDocument>) { }
  ...
  async findAll(vehiclesArgs: VehiclesArgs): Promise<Vehicle[]> {
    return this.vehiclesModel.find({}).limit(vehiclesArgs.take).skip(vehiclesArgs.skip)
  }
